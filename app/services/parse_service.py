...
from app.services.jd_parser_service import parse_job_description as parse_structured_job_description
from app.services.resume_parser_service import parse_resume as parse_structured_resume
...
def _extract_keywords(text: str, limit: int = 10) -> list[str]:
    ...
    ranked_words = Counter(filtered_words).most_common(limit)
...
def parse_resume(payload: ResumeParseRequest) -> ResumeParseResponse:
    return parse_structured_resume(payload)


def parse_job_description(
    payload: JobDescriptionParseRequest,
) -> JobDescriptionParseResponse:
    return parse_structured_job_description(payload)
...
def _parse_skills(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        value = cleaned.split(":", 1)[1].strip() if ":" in cleaned and len(cleaned.split(":", 1)[0].split()) <= 3 else cleaned
...
def _parse_work_experience(lines: list[str]) -> list[WorkExperienceItem]:
    ...
    while index < len(lines):
        ...
        if _looks_like_two_line_work_header(line, next_line):
            current = _finalize_and_start(entries, current, _build_two_line_work_item(line, next_line or ""))
            index += 2
            continue
...
def _parse_certifications(lines: list[str]) -> list[CertificationItem]:
    ...
    for line in lines:
        ...
        parts = [part.strip() for part in re.split(r"\s+\|\s+|\s+-\s+|,\s*", content) if part.strip()]
...
def _build_two_line_work_item(line: str, next_line: str) -> WorkExperienceItem:
    ...
    title = secondary if secondary else None
...
def _build_project_item(line: str) -> ProjectItem:
    parts = [part.strip() for part in re.split(r"\s+\|\s+|\s+-\s+", line, maxsplit=1) if part.strip()]
    if not parts:
...
def _extract_company_and_title(header: str) -> tuple[str | None, str | None]:
    parts = [part.strip() for part in re.split(r"\s+\|\s+|\s+at\s+|\s+-\s+|,\s*", header) if part.strip()]
    if len(parts) < 2:
        return (parts[0], None) if parts and _looks_like_company(parts[0]) else (None, parts[0] if parts and _looks_like_title(parts[0]) else None)
...
def _looks_like_name(line: str) -> bool:
    if any(token in line.lower() for token in ("@", "linkedin", "github", "http", ".com")):
        return False
...
def _looks_like_education_header(line: str) -> bool:
    if BULLET_PREFIX_PATTERN.match(line):
        return False
    if _looks_like_company(line) or _looks_like_degree(line) or _extract_date_range(line):
        return True
    return False
